...
cd $rdir/system/vold && git fetch slimroms && git merge slimroms/lp5.0
echo -e "${bldgrn}Updating hardware/qcom/camera ${txtrst}"
cd $rdir/hardware/qcom/camera && git fetch slimroms && git merge slimroms/lp5.0
...
cd $rdir/packages/apps/InCallUI && git fetch slimroms && git merge slimroms/lp5.0
echo -e "${bldgrn}Updating packages_services_Telephony ${txtrst}"
cd $rdir/packages/services/Telephony && git fetch slimroms && git merge slimroms/lp5.0
echo -e "${bldgrn}Updating packages_apps_Mms ${txtrst}"
cd $rdir/packages/apps/Mms && git fetch slimroms && git merge slimroms/lp5.0
echo -e "${bldgrn}Updating device/lge/mako ${txtrst}"
cd $rdir/device/lge/mako && git fetch slimroms && git merge slimroms/lp5.0
echo -e "${bldgrn}Updating device/lge/hammerhead ${txtrst}"
cd $rdir/device/lge/hammerhead && git fetch slimroms && git merge slimroms/lp5.0
